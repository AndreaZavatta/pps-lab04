...
object SchoolModel:
  ...
  object BasicSchoolModule extends SchoolModule:
    ...
    extension (school: School)
      def courses: Sequence[String] = school.map((x,y) => y).distinct()
      def teachers: Sequence[String] = school.map((x,y) => x).distinct()
      def setTeacherToCourse(teacher: Teacher, course: Course): School = school.concat(Cons((teacher, course), Nil()))
      def coursesOfATeacher(teacher: Teacher): Sequence[Course] = school.filter((x,y) => x == teacher).map((x,y) => y)
      def hasTeacher(name: String): Boolean = school.filter((x,y) => x == name) != Nil()
      def hasCourse(name: String): Boolean = school.filter((x,y) => y == name) != Nil()
